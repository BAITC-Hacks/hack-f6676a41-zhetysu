...
const graph=new MoneyGraph($('canvas'),$('flow-canvas'),id=>selectNode(id,true,'graph'));
function navigationState(){const step=Number(history.state?.moneyStep)||0;$('graph-back').disabled=step<=0;$('focus-current').disabled=!selected;$('focus-current').classList.toggle('active',!!selected&&graph.local);$('reset').classList.toggle('active',!graph.local)}
...
function savePosition(replace=false){
 ...
 if(selected&&!graph.local)u.searchParams.set('scope','all');else u.searchParams.delete('scope');
 ...
 const previousStep=Number(history.state?.moneyStep)||0;
 if(u.href!==location.href)history[replace?'replaceState':'pushState']({moneyStep:replace?previousStep:previousStep+1},'',u);
 navigationState();
}
function tab(name,persist=true){
 if(!['network','top','clusters','terminal','ask'].includes(name))name='network';
 const changed=currentTab!==name;currentTab=name;document.querySelectorAll('.view').forEach(el=>el.hidden=el.id!==name);
 ...
 if(matchMedia('(max-width:760px)').matches){const nav=document.querySelector('.case-navigation'),active=nav.querySelector('.active');if(active)nav.scrollTo({left:active.offsetLeft-12,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'})}
 if(name==='network'&&changed)requestAnimationFrame(()=>{graph.resize();graph.request()});
 if(persist)savePosition();
}
function selectNode(id,persist=true,source='controls'){
 if(typeof id!=='string'||!byId.has(id)){msg('Клиент с gid '+id+' не найден. Проверьте полный номер.',true);return}
 const mobileGraphAnchor=source==='graph'&&matchMedia('(max-width:760px)').matches?$('canvas').getBoundingClientRect().top:null;
 const unchanged=selected===id&&graph.local&&currentTab==='network';if(unchanged){navigationState();return}
 ...
 msg('Зелёные стрелки — поступления, синие — исходящие. Нажмите клиента, чтобы продолжить разбор.');
 if(mobileGraphAnchor!==null)requestAnimationFrame(()=>{const shift=$('canvas').getBoundingClientRect().top-mobileGraphAnchor;if(Math.abs(shift)>1)window.scrollBy({top:shift,behavior:'instant'})});
 if(persist){savePosition();if(matchMedia('(max-width:760px)').matches&&source!=='graph')requestAnimationFrame(()=>$('dossier').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'}));}
}
...
function restorePosition(){
 ...
 if(id&&byId.has(id)){selectNode(id,false);if(u.searchParams.get('scope')==='all'){graph.filter('',false);updateGraphTitle();}}
 ...
 tab(u.searchParams.get('view')||'network',false);restoring=false;navigationState();
...
function updateGraphTitle(){
 ...
 $('map-note').textContent=selected&&graph.local?'Плательщики → выбранный клиент → получатели':'Обзор сети · приблизьте или выберите клиента';
 $('legend').innerHTML=(selected?'<span><i class="legend-line"></i> Входящие к клиенту</span><span><i class="legend-line outgoing"></i> Исходящие от клиента</span><span><i class="legend-line path"></i> Путь от исходных</span>':'<span><i class="legend-line path"></i> Перевод по стрелке</span>')+'<span><i class="legend-seed"></i> Исходный клиент</span><span><i class="legend-signal"></i> Признак для проверки</span>';
 navigationState();
}
function overview(){
...
function dossier(n){
 ...
 const facts=[['Индекс приоритета',pct(n.priority_score)],['Оценка роли',pct(n.role_score)],['Входящих переводов',fmt(n.in_tx)],['Исходящих переводов',fmt(n.out_tx)],['Доля пропуска',n.pass_through==null?'Нет входящих':pct(n.pass_through)],['Колено обхода',n.depth],['Исходный клиент',n.is_seed?'Да':'Нет'],['Сообщество',n.cluster_id],['Наблюдение',terminalStatuses[n.terminal_status]?.label||'Не рассчитано']];
 const boundary=n.terminal_status==='unknown_truncated'||n.truncated_by_depth,decision=decisions[n.gid];
 const signals=[];
 if(n.flag_fast_transit)signals.push('Быстрый транзит');
 if(n.flag_sync_collection)signals.push('Синхронный сбор');
 if(n.flag_structuring)signals.push('Дробление поступлений');
 const reason=(ranked?.why||n.why||n.evidence||'Обоснование не предоставлено. Сопоставьте наблюдаемые связи.').split('. Поднят по:')[0].replaceAll('seed:','из исходного списка:').replaceAll('осело ','разница входа и выхода ').replace(/(\d)\.(\d)/g,'$1,$2');
 const reasons=reason.split(';').map(t=>t.trim().replace(/[.]$/,'')).filter(t=>t&&!t.includes('сквозной транзит')&&!t.includes('дробление:')).slice(0,3);
 $('dossier').innerHTML='<div class="node-kicker">GID · ИДЕНТИФИКАТОР КЛИЕНТА</div><h1 class="node-id"><span>'+esc(n.gid)+'</span><button id="copy-gid" class="copy-gid" title="Скопировать полный gid" aria-label="Скопировать полный gid">⧉</button></h1><span class="hypothesis-label">Предполагаемая роль</span><h2 class="role-heading">'+esc(role(n.role))+'</h2><h3 class="reason-heading">Основание для проверки</h3><ol class="evidence-points">'+reasons.map((t,i)=>'<li><b>'+String(i+1).padStart(2,'0')+'</b><span>'+esc(t.charAt(0).toUpperCase()+t.slice(1)).replaceAll('KZT','₸')+'</span></li>').join('')+'</ol>'+
 (signals.length?'<div class="signal-list">'+signals.map(t=>'<span class="signal-pill">'+esc(t)+'</span>').join('')+'</div><p class="signal-caption">Признаки поведения клиента. Нужна проверка операций.</p>':'')+
 ...
 '<div class="inspector-actions"><button id="neighbors">Связи клиента</button><button id="show-on-map">Показать во всей сети ↗</button></div>'+
 '<details class="case-detail"><summary>Обоснование роли и показатели</summary><div class="evidence">'+esc(n.evidence||'Нет дополнительных оснований')+'</div><p class="graph-access-note">Оценка роли показывает соответствие признакам, а не вероятность виновности. Быстрый транзит оценивает временные связи поступлений и исходящих, а не долю всех поступлений, переданных дальше.</p><div class="facts">'+facts.map(([k,v])=>'<div><span>'+esc(k)+'</span><strong>'+esc(v)+'</strong></div>').join('')+'</div><p class="evidence">'+esc(n.rule_id||'')+'</p>'+(n.is_seed||n.pass_through>1?'<p class="note">Внешние поступления не видны. Эти суммы не отражают полный баланс клиента.</p>':'')+'</details>'+
 '<details class="case-detail"><summary>Переводы списком · '+edges.length+'</summary>'+([...edges].sort((a,b)=>b.sum_kzt-a.sum_kzt).map(e=>{const incoming=e.dst===n.gid,other=incoming?e.src:e.dst;return '<button class="neighbor" data-gid="'+esc(other)+'">'+(incoming?'↙ От ':'↗ К ')+esc(other)+'<span>'+money(e.sum_kzt)+' · '+fmt(e.n_tx)+' переводов</span></button>'}).join('')||'<p class="muted">В предоставленной выборке переводов нет. Это не означает отсутствия операций вне неё.</p>')+'</details>';
 const flow=$('dossier').querySelector('.funds-flow');$('dossier').querySelector('.role-heading').after(flow);
 $('review-actions').hidden=false;$('review-actions').innerHTML='<div class="case-decision"><button id="mark-check" class="primary">'+(decision==='check'?'✓ В проверке':'Взять в проверку →')+'</button><button id="mark-later">'+(decision==='later'?'✓ Отложен':'Отложить')+'</button></div><p class="decision-note '+(decision?'saved':'')+'">'+(decision?'Отметка сохранена на этом устройстве. <button id="clear-decision" class="decision-clear">Снять</button>':'Сохранится на этом устройстве. Не блокирует счета.')+'</p>';
 const mark=value=>{if(value)decisions[n.gid]=value;else delete decisions[n.gid];try{localStorage.setItem('money-review',JSON.stringify(decisions))}catch(_){msg('Браузер не разрешил сохранить отметку. Она действует до закрытия страницы.',true)}const scroll=$('dossier').scrollTop;dossier(n);$('dossier').scrollTop=scroll;renderQueue()};
 $('mark-check').onclick=()=>mark('check');$('mark-later').onclick=()=>mark('later');if($('clear-decision'))$('clear-decision').onclick=()=>mark(null);
 $('copy-gid').onclick=async()=>{try{await navigator.clipboard.writeText(n.gid);msg('Полный gid скопирован: '+n.gid)}catch(_){msg('Копирование недоступно. Выделите и скопируйте gid в досье.',true)}};
 $('neighbors').onclick=()=>{tab('network');graph.select(n.gid);updateGraphTitle();savePosition()};
 $('show-on-map').onclick=()=>{tab('network');graph.filter('',false);updateGraphTitle();$('graph-title').textContent='Клиент в структуре сети';savePosition()};
}
...
function tables(){
 ...
 $('legend').innerHTML='<span><i class="legend-line"></i> Входящие</span><span><i class="legend-line outgoing"></i> Исходящие</span><span><i class="legend-line path"></i> Путь от исходных</span><span><i class="legend-seed"></i> Исходный клиент</span><span><i class="legend-signal"></i> Признак для проверки</span>';
}
...
async function load(options={}){
 if(loading)return;loading=true;$('reload').disabled=true;const quiet=options.quiet===true||!!data,previous={selected,cluster:$('cluster').value,tab:currentTab,removal:removalCount,local:graph.local};if(!quiet)msg('Загрузка и проверка данных…');const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),20000);
...
$('graph-back').onclick=()=>{if((Number(history.state?.moneyStep)||0)>0)history.back()};
$('focus-current').onclick=()=>{if(selected){graph.select(selected);updateGraphTitle();savePosition()}};
...
const sectionNavigation=document.querySelector('.case-navigation');
function placeSearch(){if(mobileLayout.matches){document.querySelector('.workbench').insertBefore(searchHeader,document.querySelector('.workspace'));document.querySelector('.workbench').insertBefore(sectionNavigation,document.querySelector('.workspace'))}else{document.querySelector('.workspace').prepend(searchHeader);searchHeader.after(sectionNavigation)}}
mobileLayout.addEventListener('change',placeSearch);placeSearch();
window.addEventListener('offline',()=>msg('Нет сети. Загруженный граф доступен; новые вопросы требуют соединения.',true));
window.addEventListener('online',()=>msg('Соединение восстановлено. Материалы доступны.'));
if(document.fonts)document.fonts.ready.then(()=>graph.request());
