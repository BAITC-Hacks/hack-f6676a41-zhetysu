...
class ApprovedGraph {
 constructor(canvas, options = {}) {
  ...
  this.view={gid:null,mode:'focus',clusterId:null,direction:'all',amounts:false};
 ...
 setView(next={}){
  const old=this.view,view={...old,...next};
  ...
  const key=JSON.stringify([view.gid,view.mode,view.clusterId,view.direction,[...this.removed].sort()]);
  if(key===this.sceneKey){this.request();return;}
  this.sceneKey=key;this.rebuild(true);
 }
 refreshTheme(){
 ...
 resize(){
  ...
  if(this.userCamera){this.camera.x+=(this.width-oldW)/2;this.camera.y+=(this.height-oldH)/2;this.request();}else this.rebuild(false);
 }
 focusTargets(){
  ...
  const relevant=(this.adj.get(gid)||[]).filter(e=>!this.removed.has(e.src)&&!this.removed.has(e.dst)&&(this.view.direction==='all'||(this.view.direction==='in'?e.dst===gid:e.src===gid)));
  const weights=new Map();for(const e of relevant){const other=e.src===gid?e.dst:e.src;if(other===gid)continue;weights.set(other,(weights.get(other)||0)+(Number(e.sum_kzt)||0));if(e.dst===gid)this.directIn.add(other);if(e.src===gid)this.directOut.add(other);}
  ...
  const ins=incoming.slice(0,rows*columns),outs=outgoing.slice(0,rows*columns);
  // The central node has room for its full 18-character identifier and hypothesis.
  const compact=h<300,center={x:w*(mobile?.48:.43),y:Math.max(58,Math.min(h-(compact?94:117),h*(compact?.38:.43))),core:1};targets.set(gid,center);
  const place=(ids,side)=>{const perColumn=Math.min(rows,Math.max(1,ids.length));ids.forEach((id,i)=>{const col=Math.floor(i/perColumn),count=Math.min(perColumn,ids.length-col*perColumn),row=i%perColumn;
    const start=mobile?48:43,available=Math.max(0,h-(mobile?122:98)),step=count>1?Math.min(mobile?70:59,available/(count-1)):0;
    const groupHeight=step*(count-1),y=start+Math.max(0,(available-groupHeight)/2)+row*step;
    const x=side<0?w*(mobile?.11:.085)+col*(w*.14):w*(mobile?.84:.72)+col*(w*.15);
    targets.set(id,{x,y,core:0});});};
  place(ins,-1);place(outs,1);
  const ids=new Set(targets.keys()),edges=relevant.filter(e=>ids.has(e.src)&&ids.has(e.dst));
  return{targets,edges,total:weights.size,allEdges:relevant.length};
 }
 overviewTargets(){
  ...
  const targets=new Map(),w=this.width,h=this.height;if(!list.length)return{targets,edges:[],total:0};
  ...
  const scale=Math.min(Math.max(40,w-90)/Math.max(.05,x1-x0),Math.max(60,h-105)/Math.max(.05,y1-y0));
  for(const n of list)targets.set(n.gid,{x:w/2+(n.x-(x0+x1)/2)*scale,y:h/2-8-(n.y-(y0+y1)/2)*scale,core:0});
  const selected=this.view.gid,all=this.sortedEdges.filter(e=>targets.has(e.src)&&targets.has(e.dst)&&(this.view.direction==='all'||!selected||(this.view.direction==='in'?e.dst===selected:e.src===selected)));
  // Canvas handles all clients; reduce only background lines at overview density.
  const cap=w<520?350:850,connected=all.filter(e=>e.src===selected||e.dst===selected),seen=new Set(connected),edges=connected.concat(all.filter(e=>!seen.has(e)).slice(0,cap));
  return{targets,edges,total:list.length,allEdges:all.length};
 }
 rebuild(animate=true){
  ...
  const snapshot=new Map();for(const[id,p]of this.positions)snapshot.set(id,{...this.project(p),alpha:p.alpha,core:p.core,dot:p.dot??(this.view.mode==='focus'?11:2.3),size:(p.size||1)*Math.min(2.4,Math.max(.7,Math.sqrt(this.camera.scale)))});
  const oldEdges=this.renderEdges,scene=this.view.mode==='focus'?this.focusTargets():this.overviewTargets();
  const anchor=snapshot.get(this.view.gid)||{x:this.width*.43,y:this.height*.43};this.camera={x:0,y:0,scale:1};this.userCamera=false;
  const next=new Map(),transitions=new Map(),nextEdges=new Map(),edgeTransitions=new Map();
  for(const[id,target]of scene.targets){const previous=snapshot.get(id),from=previous||{...anchor,alpha:0,core:target.core,size:1,dot:this.view.mode==='focus'?11:2.3};next.set(id,{...from,n:this.nodes.get(id)});transitions.set(id,{from,to:{...target,alpha:1,size:1,dot:this.view.mode==='focus'?11:2.3}});}
  for(const[id,p]of snapshot)if(!scene.targets.has(id)&&p.alpha>.005){next.set(id,{...p,n:this.nodes.get(id)});transitions.set(id,{from:p,to:{...p,alpha:0}});}
  ...
  this.stats={visibleNodes:scene.targets.size,visibleEdges:scene.edges.length,totalNeighbors:scene.total,hiddenNeighbors:this.view.mode==='focus'?Math.max(0,scene.total-scene.targets.size+1):0,totalEdges:scene.allEdges||scene.edges.length};
  const statsKey=JSON.stringify(this.stats);if(statsKey!==this.statsKey){this.statsKey=statsKey;this.options.onStats?.({...this.stats});}
  this.transition={start:now,duration:animate&&!this.reduced&&snapshot.size?580:0,nodes:transitions,edges:edgeTransitions};this.hover=null;this.hoverEdge=null;this.advance(now);this.reportZoom();this.request();
 }
 advance(now){
 ...
 radius(p){const local=this.view.mode==='focus';return (p.size||1)*Math.min(2.4,Math.max(.7,Math.sqrt(this.camera.scale)))*((p.dot??(local?11:2.3))+p.core*(this.height<300?19:this.width<520?18:23));}
 ...
 paint(now){
  ...
  const local=this.view.mode==='focus',selected=this.view.gid;
  for(const[e,state]of this.renderEdges){
   if(state.alpha<.01||this.removed.has(e.src)||this.removed.has(e.dst))continue;const path=this.edgePath(e);if(!path)continue;
   const hover=this.hoverEdge===e||(this.hover&&(e.src===this.hover||e.dst===this.hover));
   const focused=e.src===selected||e.dst===selected,alpha=state.alpha*(local?.88:focused?.9:.23)*(this.hover&&!hover?.22:1),importance=Math.log1p(Number(e.sum_kzt)||0)/Math.log1p(this.maxAmount);
   ...
   if(local||focused||this.camera.scale>1.7){const dx=path.b.x-path.c2.x,dy=path.b.y-path.c2.y,len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len,s=local?7:4;
   ...
   if(state.alpha>.5&&(local||focused))this.paths.push({...path,e});
  }
  // Amounts are intentionally optional; labels avoid central badges and one another.
  if(local&&this.view.amounts)for(const path of this.paths.slice(0,20)){const p=this.at(path,path.e.dst===selected?.46:.64),text=this.money(path.e.sum_kzt);c.font='500 10px '+this.font;const width=c.measureText(text).width+10,box={x:p.x-width/2,y:p.y-16,w:width,h:17};if(box.x<4||box.x+box.w>w-4||box.y<26||box.y+box.h>h-44||this.labelBoxes.some(b=>this.overlap(b,box)))continue;this.labelBoxes.push(box);c.globalAlpha=.92;c.fillStyle=col.canvas;this.roundRect(box.x,box.y,box.w,box.h,4);c.fill();c.globalAlpha=1;c.fillStyle=this.edgeColor(path.e,true);c.fillText(text,box.x+5,box.y+12);}
  const points=[...this.positions.entries()].sort((a,b)=>a[1].core-b[1].core);
  for(const[id,p]of points){if(p.alpha<.01)continue;const q=this.project(p),r=this.radius(p);if(q.x< -160||q.x>w+160||q.y< -140||q.y>h+140)continue;
   ...
   c.globalAlpha=p.alpha*(this.hover&&this.hover!==id&&!this.isNeighbor(this.hover,id)?.4:1);
   ...
   c.fillStyle=local?(incoming?col.blueSoft:col.greenSoft):(p.n.is_seed?col.green:col.soft);c.strokeStyle=local?color:col.panel;c.lineWidth=local?1.5:0;c.beginPath();c.arc(q.x,q.y,r,0,Math.PI*2);c.fill();if(local)c.stroke();if(p.core>.001){const alpha=c.globalAlpha;c.globalAlpha*=p.core;c.fillStyle=col.green;c.fill();c.globalAlpha=alpha;}
   if(local&&p.core<.5){c.fillStyle=color;c.beginPath();c.arc(q.x,q.y,Math.max(2,r*.27),0,Math.PI*2);c.fill();}
   if(!local&&isSelected){c.strokeStyle=col.green;c.lineWidth=2;c.beginPath();c.arc(q.x,q.y,r+5,0,Math.PI*2);c.stroke();}
   if(p.core>.05)this.paintCore(p,q,r);
   if(signal&&(local||isSelected)){const sx=q.x+r*.85,sy=q.y-r*.85,s=3.6;c.fillStyle=col.panel;c.strokeStyle=col.amber;c.lineWidth=1.5;c.beginPath();c.moveTo(sx,sy-s);c.lineTo(sx+s,sy);c.lineTo(sx,sy+s);c.lineTo(sx-s,sy);c.closePath();c.fill();c.stroke();}
   if(local&&p.core<.7)this.paintNodeLabel(p,q,reciprocal,incoming);
  }
 ...
 hit(x,y){let result=null,best=Infinity;for(const[id,p]of this.positions){if(p.alpha<.5||!this.visibleIds?.has(id)||this.removed.has(id))continue;const q=this.project(p),d=Math.hypot(x-q.x,y-q.y);if(d<Math.max(this.view.mode==='focus'?17:8,this.radius(p)+6)&&d<best){result=id;best=d;}}return result;}
 ...
 zoomAt(factor,x=this.width/2,y=this.height/2){const before=this.camera.scale,scale=Math.max(.45,Math.min(4,before*factor));this.camera.x=x-(x-this.camera.x)*scale/before;this.camera.y=y-(y-this.camera.y)*scale/before;this.camera.scale=scale;this.userCamera=true;this.reportZoom();this.request();}
 zoomBy(factor,x=this.width/2,y=this.height/2){if(!Number.isFinite(factor)||factor<=0)return;cancelAnimationFrame(this.inertia);this.inertia=0;this.hover=null;this.hoverEdge=null;this.zoomTarget=Math.max(.45,Math.min(4,(this.zoomTarget||this.camera.scale)*factor));this.zoomAnchor={x,y};if(this.reduced){this.zoomAt(this.zoomTarget/this.camera.scale,x,y);this.zoomTarget=null;return;}if(this.zoomFrame)return;const tick=()=>{this.zoomFrame=0;if(!this.zoomTarget||this.destroyed)return;const target=this.zoomTarget,next=this.camera.scale+(target-this.camera.scale)*.24;this.zoomAt(next/this.camera.scale,this.zoomAnchor.x,this.zoomAnchor.y);if(Math.abs(target-next)>.002)this.zoomFrame=requestAnimationFrame(tick);else{this.zoomAt(target/this.camera.scale,this.zoomAnchor.x,this.zoomAnchor.y);this.zoomTarget=null;}};this.zoomFrame=requestAnimationFrame(tick);}
 fit(){this.rebuild(true);}
 bind(){
