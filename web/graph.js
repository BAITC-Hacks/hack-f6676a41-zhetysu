...
class MoneyGraph extends MoneyGraphBase {
 ...
 localLayout(){
  ...
  const order=(a,b)=>(weights.get(b)||0)-(weights.get(a)||0)||a.localeCompare(b),ins=[...incoming].sort(order),outs=[...outgoing].filter(id=>!incoming.has(id)).sort(order);
  // Reciprocal counterparties appear once; both arrows remain visible.
  const place=(ids,side)=>{const rows=Math.min(11,Math.max(1,ids.length)),columns=Math.ceil(ids.length/rows);for(let i=0;i<ids.length;i++){const column=Math.floor(i/rows),length=Math.min(rows,ids.length-column*rows),row=i%rows;targets.set(ids[i],{x:side*(285+column*165),y:(row-(length-1)/2)*68})}return{x:side*(285+Math.max(0,columns-1)*82.5),y:-((rows-1)*68/2)-66,count:ids.length}};
  this.lanes={incoming:place(ins,-1),outgoing:place(outs,1)};
  const context=this.visible.filter(p=>!targets.has(p.n.gid)).sort((a,b)=>(a.n.depth||0)-(b.n.depth||0)||a.n.gid.localeCompare(b.n.gid)),bottom=Math.max(100,(Math.max(Math.min(11,ins.length),Math.min(11,outs.length))-1)*34+110);
  ...
  if(this.W>=640){const ys=[...targets.values()].map(p=>p.y),span=Math.max(150,Math.max(...ys)-Math.min(...ys)),sy=Math.max(85,this.H-160)/span,factor=Math.max(1,(this.W-240)*.4/(285*sy));for(const[id,p]of targets)if(incoming.has(id)||outgoing.has(id))p.x*=factor;this.lanes.incoming.x*=factor;this.lanes.outgoing.x*=factor}return targets;
 }
 fitCamera(targets){
 ...
 paintLabels(){
  ...
  for(const p of nodes){
   if(count>=24)break;const q=this.screen(p),selected=p.n.gid===this.selected;if(q.x<0||q.x>this.W||q.y<top||q.y>bottom)continue;const text=selected?p.n.gid:'…'+p.n.gid.slice(this.W<500?-6:-9),role=roles[p.n.role]||p.n.role;c.font='600 12px ui-monospace, monospace';const w=Math.max(c.measureText(text).width,selected?c.measureText(role).width:0)+12,h=selected?38:22,r=this.radius(p.n)+12,left={x:q.x-r-w,y:q.y-h/2},right={x:q.x+r,y:q.y-h/2},below={x:q.x-w/2,y:q.y+r},above={x:q.x-w/2,y:q.y-r-h},point=this.positions.get(p.n.gid),candidates=(selected?[below,above,right,left]:point.x<0?[left,right,below,above]:[right,left,below,above]).map(a=>({...a,w,h}));
